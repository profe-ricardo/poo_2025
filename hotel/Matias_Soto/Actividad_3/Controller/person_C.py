...
class ReceptionController:
    def _init_ (self, Model: ReceptionModel):
        self.Model = Model

    def Register_Reception(self, Name : str, Phone: int, Location: str):
        if not Name or not Phone or not Location:
            print("[####] Hacen Falta datos en la recepcion")
            return False
        
        return self.Model.register_rececption(Name, Phone, Location)
    
    def List_Reception(self) -> list:

        Reception = self.Model.Display_all()

        if len(Reception) > 0:
            return [{"Name": r[0],"Phone": r[1], "Location": r[2]} for r in Reception]
        else:
            return []
